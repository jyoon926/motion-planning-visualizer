...
export default function SidePanel() {
  const {
    algorithm, phases, params, setParams,
    setCurrentStep, activePhaseId, setHoveredPhaseId
  } = useStore();
  ...
  const algorithmInfo = {
    visibility: { title: 'Visibility Graph', complexity: 'Time: O(V² * E_obs) | Space: O(V²)' },
  ...
  return (
    <div className='p-2 pt-0'>
      <div
        className="h-full flex flex-col rounded-lg border border-black/15 shadow-lg z-10 relative flex-shrink-0 overflow-hidden"
        style={{ width }}
      >
        {/* Header */}
        <div className="p-3 border-b border-gray-200/50 bg-white/40">
        ...
        <div className="p-4 border-b border-gray-200/50 bg-gray-50/50 space-y-4">
          ...
          {algorithm === 'visibility' && (
            <div className="flex items-center gap-1">
              <label className="text-xs font-medium text-gray-700 min-w-[100px]">Obstacle Margin:</label>
              <input
                type="range" min="0" max="20" value={params.obstacleMargin}
                onChange={(e) => setParams({ ...params, obstacleMargin: Number(e.target.value) })}
                className="flex-1 h-1 bg-gray-300 rounded-lg appearance-none cursor-pointer"
              />
              <span className="text-xs text-gray-500 w-8 text-right">{params.obstacleMargin}px</span>
            </div>
          )}
          ...
          {algorithm === 'voronoi' && (
            <>
              <div className="flex items-center gap-1">
                ...
                <input
                  type="range" min="10" max="100" value={params.voronoiSampleDist}
                  onChange={(e) => setParams({ ...params, voronoiSampleDist: Number(e.target.value) })}
              ...
              <div className="flex items-center gap-1">
                ...
                <input
                  type="range" min="0" max="20" value={params.obstacleMargin}
                  onChange={(e) => setParams({ ...params, obstacleMargin: Number(e.target.value) })}
                  className="flex-1 h-1.5 bg-gray-300 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                />
                <span className="text-xs text-gray-500 w-8 text-right">{params.obstacleMargin}px</span>
              </div>
            </>
          ...
          {phases.map((phase) => {
            ...
            return (
              <div
                ...
                ${isActive ? 'border-blue-400/50 shadow-md bg-blue-50/30' : 'border-gray-200/60 bg-white/40 hover:border-blue-300/30'}
              `}
                ...
                >
                  <div className="p-4 pt-0 text-sm text-gray-600 space-y-3">
                    ...
                    <div className="bg-gray-900 rounded-lg p-3 font-mono text-xs text-gray-300 overflow-x-auto shadow-inner relative group">
                      <div className="absolute top-1 right-2 text-[10px] text-gray-500 uppercase tracking-wider">Pseudocode</div>
                      <pre className="mt-2">
                        {phase.pseudocode.split('\n').map((line, idx) => (
                          <div key={idx} className="px-1 rounded">
                            <span className="text-gray-600 select-none w-4 inline-block text-right mr-2">{idx + 1}</span>
                            {line}
                          </div>
                        ))}
                      </pre>
