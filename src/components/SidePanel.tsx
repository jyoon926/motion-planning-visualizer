  ...
  return (
    <div className="h-full flex flex-col relative flex-shrink-0 overflow-hidden" style={{ width }}>
      ...
      <div className="p-4">
        ...
        {algorithm !== 'compare' && (
          <div className="mt-2 flex items-center gap-2 font-mono text-sm bg-text/8 py-2 px-3 rounded-full">
            <MdInfoOutline className="text-text/50 text-lg" />
      ...
      {(algorithm === 'voronoi' || algorithm === 'compare') && (
        <div className="p-4 pt-0 space-y-4">
          <div className="rounded-xl border border-text/10 py-2 px-3">
            <div className="flex items-center gap-2 font-semibold mb-2">
              <MdSettings className="text-text/50" /> Parameters
            </div>
            <div className="flex items-center gap-3 text-sm text-text/50">
              ...
              <input
                ...
                className="flex-1 h-1 bg-text/8 rounded-full appearance-none accent-text"
              />
      ...
      {algorithm === 'compare' && (
        <div className="flex-1 overflow-y-auto p-4 pt-0 space-y-6">
          <div className="space-y-3">
            ...
            <div className="p-3 rounded-xl bg-text/8">
              <p className={`flex justify-between text-[#8aa656]`}>
                <span className="font-bold">Visibility Graph (Shortest Path):</span>
                <span className="font-mono text-base">{visLength} px</span>
              </p>
              <p className={`flex justify-between text-[#5190c8ff]`}>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-4">
              ...
              <p className="font-semibold">
                <span className="text-[#8aa656]">Visibility Graph</span> often yields a shorter, but more risk-prone
                path near corners, while the
                <span className="text-[#5190c8ff]"> Voronoi Diagram</span> guarantees the maximum safety margin.
              </p>
            </div>
          </div>
        </div>
      ...
      {algorithm !== 'compare' && (
        <div className="flex-1 px-4 overflow-hidden">
          <div className="h-full rounded-xl border border-text/10 overflow-hidden">
              ...
              {phases.map((phase) => {
                ...
                return (
                  <div
                    ...
                    ${isActive ? 'border-text/30 shadow-md' : 'border-text/10'}
                  `}
                  ...
                  >
                    ...
                    <button
                      ...
                      className={`w-full p-3 text-left relative ${isActive && 'border-b border-text/10'}`}
                    >
                      {/* Progress Bar */}
                      {isActive && (
                        <div
                          className="absolute bottom-0 left-0 h-1 bg-text/8 transition-all duration-200"
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className={`w-2 h-6 rounded-full ${isActive ? 'bg-text' : 'bg-text/20'}`}></div>
                          <span className={`font-bold ${isActive ? 'text-text' : 'text-text/50'}`}>{phase.name}</span>
                        </div>
                        {isExpanded ? (
                          <MdExpandLess className="text-text/50 text-lg" />
                        ) : (
                          <MdExpandMore className="text-text/50 text-lg" />
                        )}
                      </div>
                    </button>
                    ...
                    >
                      <div className="p-4 pt-3 text-text/60 space-y-3">
                        <p className="leading-relaxed">{phase.description}</p>

                        {/* Pseudocode Block */}
                        <div className="rounded-xl bg-text/8 overflow-hidden">
                          <div className="p-3 font-mono text-sm text-text/70 overflow-x-auto relative group">
                            <div className="absolute top-2 left-3 text-[10px] uppercase tracking-wider">Pseudocode</div>
                            <pre className="mt-4">
      ...
      {/* Resize Handle */}
      <div
        className="absolute right-0 top-0 w-1.5 h-full rounded-full cursor-col-resize hover:bg-text/8 transition-colors z-20"
