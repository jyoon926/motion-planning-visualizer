...
function findVisibleEdges(currP: number, vertices: Point[], polygons: Point[][]): [Point, Point][] {
  ...
  const inOrderIntersection = (
    bst: BinarySearchTree<NodeEdge>,
    node: BinarySearchTreeNode<NodeEdge> | undefined,
    wiMinus1: Point,
    wi: Point
  ): boolean => {
    if (node) {
  ...
  const isVisible = (p: Point, w: number): boolean => {
    ...
    if (incidents !== undefined && incidents.length > 1) {
      ...
      if (orientation(wprev, vertices[w], p) > 0 && orientation(vertices[w], wnext, p) > 0) {
        return false;
      }
    }
    ...
    if (w === 1 || orientation(p, vertices[w], vertices[mod(w - 1, vertices.length)]) !== 0) {
      ...
      const left = bst.leftmost(bst.root)?.data;
      if (left == undefined) {
        console.log('isVisible: undefined leftmost node', bst);
        return false;
      }
      if (left.edge == undefined) {
        console.log('isVisible: leftmost edge undefined', left);
      }
  ...
  */
  ...
  const stringToEdge = (s: string): [Point, Point] => {
    const strs = s.split(',');
    return [
      { x: parseInt(strs[0]), y: parseInt(strs[1]) },
      { x: parseInt(strs[2]), y: parseInt(strs[3]) },
    ];
  };
  ...
  */
  ...
  for (const p of polygons) {
    ...
    for (let i = 0; i < l; i++) {
      allEdges.set(getEdgeKeyString(p[mod(i - 1, l)], p[i]), [
        getEdgeKeyString(p[mod(i - 2, l)], p[mod(i - 1, l)]),
        getEdgeKeyString(p[i], p[mod(i + 1, l)]),
      ]);
  ...
  for (const e of allEdges.keys()) {
    ...
    if (currEdge == undefined) {
      console.log('line 390: stringToEdge function did not return an edge');
    }
